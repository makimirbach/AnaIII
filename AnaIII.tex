...
%Bsp.
\begin{Beispiel}
$A_k := \{k, k+1, ... \} \subset \mathbb{N}$, card($A_k) > \infty$ \\
...
$\begin{pmatrix}
      \lambda_1 & 	 &	&	& 0 \\
		 ...
		 0 &	&	&	&	\lambda_n
\end{pmatrix}$ \\
$\Rightarrow \mathscr L^n(E) = (\lambda_1 \cdotp ... \cdotp \lambda_n) \cdotp \mathscr L^n(\mathscr B_1(0))$
\end{Beispiel}
...
\emph{Behauptung:} 
\begin{center}
	$\int\limits_{\mathbb R^n\backslash \mathscr B_1(0)} f d\mathscr L^n < \infty$ $\Leftrightarrow$ $\alpha > n$  \\	
...
Es ist weiter 
\begin{center}
	$\sum\limits_{k=0}^l 2^{-k\alpha}\chi_{A_k}$ $\underrightarrow{k \rightarrow \infty}$ $\sum\limits_{k=0}^\infty 2^{-k\alpha}\chi_{A_k} = g\chi_{\mathbb R^n \backslash \mathscr B_1(0)}$
\end{center} 
...
\par
$\mathscr B_1(0) = \Set{z \in \mathbb R^n| \,|z| < 1} = \Set{z = (x, y) \in \mathbb R^{n-1} \times \mathbb R| \,|x|^2 + y^2 <1} $
\par\bigskip
$(\mathscr B_1(0))_y = \Set{x \in \mathbb R^{n-1}| \,|x| < \sqrt{1-y^2},\,  y \in (-1, 1]} = \sqrt{1-y^2}\;\mathscr B_1^{n-1} (0) $
\par\bigskip
