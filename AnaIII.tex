...
\usepackage{fdsymbol} %varrightwavearrow - blockiert Underbrace
...
\begin{Beispiel}
\textrm{ }\\[-10pt]
% Ich setzte das mal in einen Center, sonst sieht das übel doof aus. Doofe Idee, dann ist die Überschrift auch zentriert.
...
\begin{equation}
	...
	\end{cases}
\end{equation}
$0 = \int f_0 \, d\mathscr L^1 \ne \lim_{\epsilon \rightarrow 0} \int f_e \, d\mathscr L^1$ $\xlongequal{\lim_{\epsilon \searrow 0}}$ $\dfrac{1}{2\epsilon} \mathscr L^1([-\epsilon, \epsilon]) =1$
\end{Beispiel}
...
%An welchem Package kann es liegen, dass \underbrace nicht mehr funktioniert, da schwarze Balken darüber liegen? Bis letztens ging es noch. mit abrace lässt es sich nicht lösen, google kann nicht helfen.
...
$\xlongequal{\text{Satz II.10}} \lim_{l \rightarrow \infty} \sum\limits_{k=0}^l \int 2^{-k\alpha}\chi_{A_k} d\mathscr L^n = \sum\limits_{k=0}^\infty 2^{-k\alpha} \int \chi_{A_k} d\mathscr L^n = \sum\limits_{k=0}^\infty 2^{-k\alpha} \mathscr L^n (A_k)$ \\
\begin{equation}
...
\xlongequal{y = \cos{\theta}}  \alpha_{n-1} \int\limits_0^\pi \sin^n \theta \, d\theta =: \alpha_{n-1} A_n$ \\
\begin{center}
